...
fn run_fast_batch_py<'py>(
    ...
    let result: Result<(OutTuple, Vec<u8>, Vec<u8>, [u32; 22], Vec<u8>, Option<Vec<u8>>, Option<Vec<u8>>), String> =
        py.detach(move || (move || {
        ...
        std::mem::drop(mem);
        Ok((out, backing, pcm_bytes, vpack, cia2_iec_log_out, beam_vic_backing, beam_cia2_backing))
    })());
    let (out, backing_out, pcm_bytes, vpack, cia2_iec_log, beam_vic_out, beam_cia2_out) =
