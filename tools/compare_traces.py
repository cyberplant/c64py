...
IMPORTANT NOTE: Cycle numbers DO NOT match between emulators because VICE
emulates real disk loading timing while our emulator uses fast ROM loading.
For example, at PC $080D (SEI), our cycle is ~2.1M while VICE is ~79.2M.
Always compare traces by PC and register state, not cycle numbers.
Use --match-cycles-at to align traces at a specific PC for comparison.
...
class TraceLine:
    ...
    def without_cycles(self) -> str:
        """Return line without cycle count or inline comments for comparison"""
        # First strip inline comments (everything after ;), then trailing cycle count
        line = self.raw.split(';')[0]
...
def iter_trace_compare(
    binary_f: BinaryIO,
    start_byte: int = 0,
    dedupe_same_pc_opcode: bool = False,
) -> Iterator[Tuple[int, TraceLine]]:
    """``(physical_line, TraceLine)`` stream for compare/resync (same shape as ``iterate_trace_file``).

    *dedupe_same_pc_opcode*: drop consecutive rows with the same PC and opcode byte (VICE monitor
    often prints the same instruction twice at a breakpoint before the streaming trace).
    """
...
def run_emulator(
    ...
    subprocess_timeout: Optional[float] = None,
) -> bool:
...
def find_address_in_trace(
    ...
    last_dedupe: Optional[Tuple[int, int]] = None
    with open(filename, "rb") as f:
        align_lines = _prepare_binary_trace_read(f, start_byte)
...
def compare_traces_streaming(
    ...
    our_match_min_cycle: Optional[int] = None,
    vice_match_min_cycle: Optional[int] = None,
    ...
    vice_monitor_dedupe: bool = False,
) -> None:
    ...
    if match_cycles_at is not None:
        ...
        our_mmin = our_match_min_cycle if our_match_min_cycle is not None else match_min_cycle
        vice_mmin = vice_match_min_cycle if vice_match_min_cycle is not None else match_min_cycle
        if our_mmin != vice_mmin:
            print_msg(
                f"   (per-side min cycle: ours>={our_mmin if our_mmin is not None else '—'}, "
                f"vice>={vice_mmin if vice_mmin is not None else '—'})",
                "dim",
            )
        ...
        vice_anchor = find_address_in_trace(
            ...
            dedupe_same_pc_opcode=vice_monitor_dedupe,
        )
    ...
    with open(our_trace, "rb") as f_our, open(vice_trace, "rb") as f_vice:
        ...
        vice_iter: "Iterator[Tuple[int, TraceLine]]" = iter_trace_compare(
            f_vice, vice_sync_byte, dedupe_same_pc_opcode=vice_monitor_dedupe
        )
...
Large VICE logs (slow sync scan): first run prints
  ...
  Re-run with those flags so the tool seeks near the sync point instead of reading from offset 0.
    ...
    parser.add_argument(
        ...
        metavar='SEC',
        help=(
            'Subprocess timeout in seconds when compare_traces runs C64.py (default: 300). '
            'Use 0 for no limit (long Bruce-Lee–scale traces).'
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            '(skip earlier hits of the same PC). Applied to **both** traces unless '
            '--our-match-min-cycle / --vice-match-min-cycle override.'
        ),
    )
    parser.add_argument(
        '--our-match-min-cycle',
        ...
        default=None,
        metavar='N',
        help='With --match-cycles-at: min cumulative cycle on **our** trace only (overrides --match-min-cycle).',
    )
    parser.add_argument(
        '--vice-match-min-cycle',
        type=int,
        default=None,
        metavar='N',
        help='With --match-cycles-at: min cumulative cycle on **VICE** trace only (overrides --match-min-cycle).',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=(
            'When reading the VICE-side trace: skip consecutive rows with the same PC and opcode '
            '(breakpoint duplicate lines in x64sc monitor logs). Use with logs/vice_full_trace.log.'
        ),
    )
    ...
    if args.our_trace:
        ...
        emu_timeout = None if args.emulator_timeout == 0 else float(args.emulator_timeout)
        if not run_emulator(
            args.program,
            args.max_cycles,
            our_trace,
            ...
            subprocess_timeout=emu_timeout,
        ):
    ...
    compare_traces_streaming(
        ...
        our_match_min_cycle=args.our_match_min_cycle,
        vice_match_min_cycle=args.vice_match_min_cycle,
        ...
        vice_monitor_dedupe=args.vice_monitor_dedupe,
    )
