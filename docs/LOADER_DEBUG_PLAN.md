...
> **Note (2026-04):** c64py’s **`C64PY_BRUCELEE_*`** / **`C64PY_LOADER_*`** env hooks were removed; this plan and [bruce_lee_loader_investigation.md](bruce_lee_loader_investigation.md) are historical for c64py internals but still describe VICE workflow.
...
- **Loader metrics** in c64py (since **removed** from code, 2026-04): historically **`sta00fa_zp2d_before_e5f0`**, JSR counts from **`$087E`–`$0884`**, histograms — aligned with VICE on **store counts**; remaining **~46** JSR gap explained as **cycle-stamp vs instruction-boundary** semantics, not wrong destination progression.
...
- **Regen script**: [`scripts/regen_bruce_fresh_compare.sh`](../scripts/regen_bruce_fresh_compare.sh) — **deprecated** (exits 1; c64py env hooks removed).
...
3. **`LDA ($2F),Y` @ `$0113` / `$DA89` — fixed**  
   Effective address **`$DA89`** is **inside color RAM**. c64py used **4-bit** mask on CPU **`read`/`write`** there (**`$D6` → `$06`**), so **`CMP #$D6`** failed and the branch stream diverged. **Fix:** **8-bit** backing for CPU in **`$D800`–`$D8FF`** (first 1K of color region); VIC still uses low nybble when drawing. **`C64PY_BRUCELEE_WATCH_WRITES=DA89`** showed only a handful of stores, including **`STA $DA00,Y` @ `$0849`** with **`$D6`**. **Unmapped `$Dxxx` I/O** also **read/write RAM** now.
