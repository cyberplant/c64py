...
3. **`LDA ($2F),Y` @ `$0113` (root of “missing second `$010F`”)**  
   At **`idx=105705`**, both sides **`BNE` @ `$010F`** with **take=1**, then **`LDA ($2F),Y` @ `$0113`**. **VICE** loads **A=`$D6`**; **c64py** loads **A=`$06`** (same **Y=0**, eff **~`$DA89`** in the Bruce log). **VICE** **`RTS` → `$0884`**, **`CMP #$D6`**, **`BEQ $08A0`**, **`JSR $0103`**, …, second **`$010F`**. **c64py** **`RTS` → `$0884`** with **A=`$06`**: **`CMP #$D6`** clears **Z**, **`BEQ`** **not** taken — **no** **`JSR $0103`** on that visit, so **no** second inner **`$010F`**. Next logged branch is **`$088A`**. So the visible “phase slip” reduces to **wrong source byte** (or wrong **eff**) for that **`LDA`**, i.e. the same **source-pointer / copy** family as **`$E5F0`**. Next: trace **RAM @ eff**, **`$2F/$30`**, and **`INC $2F`** counts **leading into** that **`LDA`** (milestones already exist).
